...
from _database.pre_processing.routine_JRC import (
    get_jrc_balance_data_for_transport,
    get_mapping_fuels_for_transport,
    get_names_map_for_transport,
)
...
for country_code in dict_iso2_jrc.keys():
    ...
    for l in labels:
        dm_temp.append(DM_temp[l], "Variables")

    # aggregate
    dm_temp.groupby(
        {
            "aviation-passenger": [
                "aviation-passenger-domestic",
                "aviation-passenger-extra",
                "aviation-passenger-intra",
            ],
            "aviation-freight": [
                "aviation-freight-domestic",
                "aviation-freight-extra",
                "aviation-freight-intra",
            ],
...
for l in labels:
    dm_temp.append(DM_temp_country[l], "Country")
...
missing = np.array(years)[
    [y not in dm_temp.col_labels["Years"] for y in years]
].tolist()
...
dm_pas.rename_col(
    ["aviation-passenger", "rail-passenger"], ["aviation", "rail"], "Categories1"
)
dm_pas.sort("Categories1")
DM_final["passenger_jrc"] = dm_pas.copy()

dm_fre = dm_temp.filter(
    {"Categories1": ["IWW", "aviation-freight", "marine", "rail-freight", "trucks"]}
)
dm_fre.rename_col(
    ["aviation-freight", "rail-freight"], ["aviation", "rail"], "Categories1"
)
...
jrc_to_model_fuels_temp["other"] = jrc_to_model_fuels_temp["other"] + [
    "O4100TOT",
    "R5110-5150W6000RI",
    "W61006220",
]
...
dm_temp.groupby(
    {
        "aviation": ["aviation-domestic", "aviation-international"],
        "marine": ["marine-domestic", "marine-international"],
    },
    "Categories1",
    inplace=True,
