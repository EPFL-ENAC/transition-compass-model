...
powertrain_to_eurostat = {
    ...
    "BEV": ["ELC"],
    "FCEV": ["HYD_FCELL"],
    "ICE-gasoline": ["PET_X_HYB", "ELC_PET_HYB", "BIOETH"],
    "ICE-diesel": ["DIE_X_HYB", "ELC_DIE_HYB", "BIODIE"],
    "ICE-gas": ["GAS", "LPG", "BIFUEL"],
    "PHEV-gasoline": ["ELC_PET_PI"],
    "PHEV-diesel": ["ELC_DIE_PI"],
}
all_codes = [code for codes in powertrain_to_eurostat.values() for code in codes]
...
mapping_dim = {"Country": "geo", "Years": "time", "Variables": "mot_nrg"}
...
dm_new_ldv = get_data_api_eurostat_via_json(
    "road_eqr_carpda", filter, mapping_dim, "num"
)
dm_fleet_ldv = get_data_api_eurostat_via_json(
    "road_eqs_carpda", filter, mapping_dim, "num"
)

# reshape
dm_new_ldv.groupby(powertrain_to_eurostat, "Variables", inplace=True)
for v in dm_new_ldv.col_labels["Variables"]:
    dm_new_ldv.rename_col(v, "new-vehicles_LDV_" + v, "Variables")
dm_new_ldv.deepen_twice()
dm_fleet_ldv.groupby(powertrain_to_eurostat, "Variables", inplace=True)
for v in dm_fleet_ldv.col_labels["Variables"]:
    dm_fleet_ldv.rename_col(v, "stock-vehicles_LDV_" + v, "Variables")
...
dm_fleet.rename_col(
    ["FOSS", "TOTAL", "ZEMIS"], ["ICE-gasoline", "total", "BEV"], "Categories2"
)
...
powertrain_to_eurostat = {
    ...
    "BEV": ["ELC"],
    "FCEV": ["HYD_FCELL"],
    "ICE-diesel": ["DIE_X_HYB", "ELC_DIE_HYB"],
    "PHEV-diesel": ["ELC_DIE_PI"],
    "ICE-gas": ["LPG", "CNG", "LNG"],
    "ICE-gasoline": [
...
for v in dm_new.col_labels["Variables"]:
    dm_new.rename_col(v, "new-vehicles_bus_" + v, "Variables")
dm_new.deepen_twice()
dm_fleet.groupby(powertrain_to_eurostat, "Variables", inplace=True)
for v in dm_fleet.col_labels["Variables"]:
    dm_fleet.rename_col(v, "stock-vehicles_bus_" + v, "Variables")
...
mapping_dim = {"Country": "geo", "Years": "time", "Variables": "metrotram"}
...
# reshape
dm_new = add_missing(
    dm_new, country_codes, "Country"
)  # here i only have 12 countries so EU27 will be missing
...
# reshape
dm_fleet.rename_col(
    ["DIE", "ELC", "TOTAL"], ["ICE-diesel", "CEV", "total"], "Categories1"
)
dm_fleet.rename_col("LOC", "stock-vehicles_train", "Variables")
dm_fleet.deepen(based_on="Variables")
dm_fleet.switch_categories_order("Categories1", "Categories2")
dm_fleet_eu = dm_fleet.groupby(
    {"EU27_2020": dm_fleet.col_labels["Country"]}, "Country"
)  # I have 25 countries, I assume fine to aggregate to EU27
dm_fleet.append(dm_fleet_eu, "Country")
dm_fleet.sort("Country")
...
mapping_dim = {"Country": "geo", "Years": "time", "Variables": "aviation"}
...
for v in dm_fleet_total.col_labels["Variables"]:
    dm_fleet_total.change_unit(v, 1, "num", "number")
...
all_codes = [code for codes in powertrain_to_eurostat_trucks.values() for code in codes]
...
dm_new.groupby(
    {"new-vehicles_trucks": ["LOR_GT3P5", "VG_LE3P5"]}, "Variables", inplace=True
)
...
dm_fleet.groupby(
    {"stock-vehicles_trucks": ["LOR_GT3P5", "VG_LE3P5"]}, "Variables", inplace=True
)
dm_fleet.deepen(based_on="Variables")
dm_fleet.switch_categories_order("Categories1", "Categories2")
dm_fleet.append(dm_new, "Variables")
...
dm_fleet_eu = dm_fleet.groupby(
    {"EU27_2020": dm_fleet.col_labels["Country"]}, "Country"
)  # I have 18 countries, major are in, I assume I can make EU27
dm_fleet.append(dm_fleet_eu, "Country")
dm_fleet.sort("Country")
...
with open(f, "rb") as handle:
    dm_jrc_pass_fleet = pickle.load(handle)
...
with open(f, "rb") as handle:
    dm_jrc_pass_new = pickle.load(handle)

for v in dm_jrc_pass_fleet.col_labels["Variables"]:
    dm_jrc_pass_fleet.rename_col(v, "stock-vehicles_" + v, "Variables")
dm_jrc_pass_fleet.deepen(based_on="Variables")
dm_jrc_pass_fleet.switch_categories_order("Categories1", "Categories2")
for v in dm_jrc_pass_new.col_labels["Variables"]:
    dm_jrc_pass_new.rename_col(v, "new-vehicles_" + v, "Variables")
...
with open(f, "rb") as handle:
    dm_jrc_freight_fleet = pickle.load(handle)
...
dm_jrc_freight_fleet.groupby(
    {"trucks": ["HDVH", "HDVL", "HDVM"]}, "Variables", inplace=True
)
dm_fleet = dm_jrc_freight_fleet.copy()

for v in dm_fleet.col_labels["Variables"]:
    dm_fleet.rename_col(v, "stock-vehicles_" + v, "Variables")
dm_fleet.deepen(based_on="Variables")
dm_fleet.switch_categories_order("Categories1", "Categories2")
for v in dm_new.col_labels["Variables"]:
    dm_new.rename_col(v, "new-vehicles_" + v, "Variables")
dm_new.deepen(based_on="Variables")
dm_new.switch_categories_order("Categories1", "Categories2")
dm_fleet.append(dm_new, "Variables")
...
for v in dm_fleet.col_labels["Variables"]:
    dm_fleet.change_unit(v, 1, "unit", "number")
