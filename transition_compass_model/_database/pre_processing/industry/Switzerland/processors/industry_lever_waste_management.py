...
from _database.pre_processing.industry.Switzerland.get_data_functions.data_waste import get_data_waste_vehicles, get_data_special_waste, extract_waste_data_from_dechets_speciaux
...
def waste_vehicles(current_file_directory, dm_eu_vehicles, years_ots):
    ...
    dm_temp = dm_eu_vehicles.filter({"Country" : ["EU27"], "Years" : years_selection, "Categories1" : ["waste-uncollected"]})
    ...
    years_selection = dm_layer1.col_labels["Years"].copy()
    years_selection.remove(2024)
    dm_temp = dm_eu_vehicles.filter({"Country" : ["EU27"], "Years" : years_selection, "Categories1" : ['landfill','reuse']})
    dm_temp = dm_temp.flatten()
...
def waste_buildings(current_file_directory, dm_eu_bld, years_ots):
    ...
    dm_bld = extract_waste_data_from_dechets_speciaux(current_file_directory, "Déchets de chantier non triés problématiques", 'floor-area-new-residential')
    ...
    df_temp = dm_layer1.write_df()
    df_temp2 = dm_eu_bld.filter({"Country" : ["EU27"], "Years" : dm_layer1.col_labels["Years"], 
    ...
    df_temp = dm_layer2.write_df()
    df_temp2 = dm_eu_bld.filter({"Country" : ["EU27"], "Years" : dm_layer2.col_labels["Years"], 
                                    "Variables" : ["floor-area-new-residential"], 
...
def waste_roads(current_file_directory, dm_eu_roads):
    ...
    dm_roads = extract_waste_data_from_dechets_speciaux(current_file_directory, "Matériaux bitumineux de démolition des routes >","road")
    ...
    dm_layer1.normalise("Categories1")
    df_temp = dm_layer1.write_df()
    df_temp2 = dm_eu_roads.filter({"Country" : ["EU27"], "Years" : dm_layer1.col_labels["Years"], 
                                    "Variables" : ["road"], 
                                    "Categories1" : dm_layer1.col_labels["Categories1"]}).write_df()
    # seems ok
    ...
    dm_layer2.normalise("Categories1")
    df_temp = dm_layer2.write_df()
    df_temp2 = dm_eu_roads.filter({"Country" : ["EU27"], "Years" : dm_layer2.col_labels["Years"], 
                                    "Variables" : ["road"], 
                                    "Categories1" : dm_layer2.col_labels["Categories1"]}).write_df()
...
def waste_appliances(dm_eu):
    ...
    df_temp = dm_ele.filter({"Categories1" : ["waste-collected","waste-uncollected","export","littered"]}).write_df()
    # all years: collected 0.8, uncollected 0.2, can be changed to 0.95 and 0.05
    df_temp = dm_ele.filter({"Categories1" : ["recycling","incineration","energy-recovery","reuse","landfill"]}).write_df()
    ...
    dm_ele[...,"export"] = 0 # assuming export zero: The export and import of such waste requires the authorisation of the FOEN. Export to states that are not members of the OECD or EU is prohibited.
    dm_ele[...,"littered"] = 0

    # appliances
    dm_app = dm_eu.filter({"Country" : ["EU27"], "Variables" : ["domapp"]})
    dm_app.rename_col("EU27","Switzerland","Country")
    df_temp = dm_app.filter({"Categories1" : ["waste-collected","waste-uncollected","export","littered"]}).write_df()
    # same than electronics
    df_temp = dm_app.filter({"Categories1" : ["recycling","incineration","energy-recovery","reuse","landfill"]}).write_df()
    ...
    dm_app[...,"export"] = 0 # assuming export zero: The export and import of such waste requires the authorisation of the FOEN. Export to states that are not members of the OECD or EU is prohibited.
    dm_app[...,"littered"] = 0
...
def waste_pack_paper(dm_eu):
    ...
    dm = dm_eu.filter({"Country" : ["EU27"], "Variables" : ['paper-pack','paper-san','paper-print']})
    dm.rename_col("EU27","Switzerland","Country")
    ...
    mills_input  = 835_333     # domestic mills' recovered paper input (includes imports_rec)
    hygiene      = 149_706     # non-recyclable tissue/hygiene (typically ends in residual waste)
    ...
    waste_collected_share = 1.0 - export_share - littered_share - waste_uncollected_share
    waste_collected_share = max(0.0, min(1.0, waste_collected_share))  # clamp for safety
    ...
    recycling_share = 0.0 if waste_collected_mass == 0 else domestic_to_recycling_mass / waste_collected_mass
    recycling_share = max(0.0, min(1.0, recycling_share))  # clamp

    dm[...,"export"] = export_share 
    dm[...,"littered"] = littered_share
    dm[...,"waste-uncollected"] = waste_uncollected_share # assuming in CH all waste is collected
