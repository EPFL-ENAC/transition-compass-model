...
from _database.pre_processing.industry.Switzerland.get_data_functions.data_product_net_import import get_import_export_chf, get_price_data, get_price_index_data, get_io_data
...
def import_export_chf(current_file_directory):
    ...
    df_imp_exp = pd.concat([get_import_export_chf(current_file_directory, t, df_map_cpa) for t in ["exp","imp"]])
...
def make_net_import_share(current_file_directory, dm_io, years_ots):
    ...
    dm_wwp_demand = dm_netimp.filter({"Variables" : ["demand"], "Categories1" : ["wwp"]}) # I need this for fxa
    ...
    dm_netimp_goods[...,"planes_ICE"] = 0.95 # almost all imported (little local production)
    ...
    dm_netimp_goods[...,"dishwasher"] = 0.99 # almost all imported (little local production)
    dm_netimp_goods[...,"dryer"] = 0.99 # almost all imported (little local production)
    dm_netimp_goods[...,"freezer"] = 0.99 # almost all imported (little local production)
    dm_netimp_goods[...,"fridge"] = 0.99 # almost all imported (little local production)
    dm_netimp_goods[...,"wmachine"] = 0.99 # almost all imported (little local production)
...
def make_material_production(current_file_directory, dm_io, dm_wwp_demand, df_price_ts, years_ots, years_fts):
    ...
    missing = np.array(years_ots)[[y not in dm_out_calib.col_labels["Years"] for y in years_ots]].tolist()
...
def run(years_ots, years_fts):
    ...
    # data on chf / tonne
    df_price = pd.concat([get_price_data(current_file_directory, flow) for flow in ["EXP","IMP"]])
    ...
    df_io = pd.concat([get_io_data(current_file_directory, y) for y in ["2005","2008","2011","2014","2017"]])
    
    # make dm io
    dm_io = make_dm_io(df_map_cpa, df_io, df_imp_exp)
    
    # make goods net import share
    dm_netimp, dm_netimp_goods, dm_wwp_demand = make_net_import_share(current_file_directory, dm_io, years_ots)
