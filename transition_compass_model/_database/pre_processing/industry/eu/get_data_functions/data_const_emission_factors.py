...
def get_emission_factors_data(current_file_directory):
...
def get_process_emission_factors_data(current_file_directory):
    # From chatGPT:
    ...
    def get_data(filepath, tech_code, tech, tech_calc, years):
        ...
        if not any(ls_temp):
            ...
            ls_temp = [
                my_search("Process emissions", i) for i in df_temp.loc[:, id_var]
            ]
    ...
    df_temp = pd.DataFrame(
        {
            "variable": ["process-emissions_steel-hydrog-DRI_CO2[Mt/Mt]"],
            "value": [value],
        }
    )
    df = pd.concat([df, df_temp])
    ...
    df_temp = df.loc[
        df["variable"] == "process-emissions_cement-dry-kiln_CO2[Mt/Mt]", :
    ]
    ...
    # TODO: check the literature and re-do this
    df_temp = df.loc[
        df["variable"] == "process-emissions_glass-glass-tech_CO2[Mt/Mt]", :
    ]
