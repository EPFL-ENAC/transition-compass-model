...
def get_const_energy_demand_dm(current_file_directory):
    ...
    df_prod_extramat = df_prod_extramat.groupby(["variable"], as_index=False)[
        "value"
    ].agg(np.mean)
    ...
    def get_energy_intensity(
        ...
        for energy_key in DF.keys():
            ...
            for i in range(0, len(techs)):
                ...
                df_temp = df_temp.groupby([id_var], as_index=False)["value"].agg(
                    np.mean
                )
                ...
                for key in dict_adj_biomass_waste.keys():
                    ...
                    df_temp = pd.concat(
                        [
                            df_temp,
                            pd.DataFrame({"energy_carrier": key, "value": adj_value}),
                        ]
                    )
                df_temp = df_temp.loc[
                ...
                df_temp = pd.concat(
                    [
                        df_temp,
                        pd.DataFrame({"energy_carrier": ["hydrogen"], "value": [0]}),
                    ]
                )
    ...
    df_dri["total"] = 3.48
    df_dri.loc[
        df_dri["variable"] == "steel-hydrog-DRI_electricity[TWh/Mt]", "value"
    ] = 0.5
    df_dri.loc[df_dri["variable"] == "steel-hydrog-DRI_hydrogen[TWh/Mt]", "value"] = 0.5
    df_dri["value"] = df_dri["value"] * df_dri["total"]
    df = pd.concat(
        [df, df_dri.loc[:, ["variable", "value", "tech", "energy_demand_type"]]]
    )
    ...
    df["energy_carrier"] = [
        tech + "_" + ec + "[TWh/Mt]"
        for tech, ec in zip(df["tech"], df["energy_carrier"])
    ]
    ...
    df_final_feedstock.sort_values(
        ["tech", "energy_demand_type", "variable"], inplace=True
    )
    ...
    df_temp["variable"] = [
        i.replace("ois-tech", "ois-sec") for i in df_temp["variable"]
    ]
    df = pd.concat([df, df_temp])
    ...
    df_temp["variable"] = [
        i.replace("wwp-tech", "wwp-sec") for i in df_temp["variable"]
    ]
    df = pd.concat([df, df_temp])
