...
import numpy as np
import os
__file__ = "/Users/echiarot/Documents/GitHub/2050-Calculators/PathwayCalc/_database/pre_processing/industry/eu/eol_preprocessing/TV_PV.py"
...
df_tv['geo\\TIME_PERIOD'] = df_tv['geo\\TIME_PERIOD'].apply(lambda x: convert_country_code_to_name(x) if isinstance(x, str) and x != 'EU27' else x)
...
#df_tv.to_excel('/Users/sqiao/Documents/Calculators/Julie_Calc/end-of-life/pre-processing/_data-processing/data/TV_PV/Python-converted_tv.xlsx', index=False)
...
#merged_df.to_excel('/Users/sqiao/Documents/Calculators/Julie_Calc/end-of-life/pre-processing/_data-processing/data/TV_PV/Python-converted_merge.xlsx', index=False)
...
geoscale_values = ["Liechtenstein", "Norway", "Iceland", "UK", "Cyprus", "Spain", "Portugal"]
...
df.loc[(df['geoscale'] == 'Italy'), 'export[t]'] = df.loc[(df['geoscale'] == 'Italy'), 'export[t]'].fillna(0)
df.loc[(df['geoscale'] == 'Croatia'), 'export[t]'] = df.loc[(df['geoscale'] == 'Croatia'), 'export[t]'].fillna(0)
df.loc[(df['geoscale'] == 'Poland') & (df['timescale'] != 2018), :] = df.loc[(df['geoscale'] == 'Poland') & (df['timescale'] != 2018), :].fillna(0)
...
if italy_data['waste-collected[t]'].notna().sum() > 1:
    ...
    for column in [col for col in italy_data.columns if col not in ['waste-collected[t]', 'timescale', 'geoscale']]:
        ...
        if italy_data[column].isna().any():
            ...
            if len(clean_data) > 1:
                x = clean_data['waste-collected[t]'].values.reshape(-1, 1)  # Independent variable
                ...
                x_pred = italy_data.loc[italy_data[column].isna(), 'waste-collected[t]'].values.reshape(-1, 1)
                italy_data.loc[italy_data[column].isna(), column] = model.predict(x_pred)
    ...
    if geo not in ['Italy', 'EU27', 'Croatia', 'Slovenia'] + geoscale_values:
        ...
        for column in geo_data.select_dtypes(include=[np.number]).columns:
            if geo_data[column].notna().sum() > 1:  # Ensure at least two data points
                slope, intercept = np.polyfit(geo_data.loc[geo_data[column].notna(), 'timescale'], geo_data.loc[geo_data[column].notna(), column], 1)
...
excluded_geoscales = ['Liechtenstein', 'Norway', 'Iceland', 'UK'] # Excluding non-EU27 countries
sum_data = df[~df['geoscale'].isin(excluded_geoscales + ['EU27'])] # Exclude specific geoscales and drop specified columns before summing
summed_data = sum_data.groupby('timescale').sum(min_count=1).reset_index() # Sum data grouped by 'timescale', with handling for NaN values
sum_dict = summed_data.set_index('timescale').to_dict('index')
eu27_data = df[df['geoscale'] == 'EU27'].copy() # Filling values for EU27 explicitly for all numeric columns
...
for column in eu27_data.select_dtypes(include=[np.number]).columns:
    if column not in ['geoscale', 'timescale']:  # Exclude non-data columns
        ...
        eu27_data[column] = eu27_data.apply(
            lambda row: sum_dict.get(row['timescale'], {}).get(column, np.nan) if pd.isna(
                row[column]) else sum_dict.get(row['timescale'], {}).get(column, row[column]),
            axis=1
        )
...
# Check
#df.to_excel('/Users/sqiao/Documents/Calculators/Julie_Calc/end-of-life/pre-processing/_data-processing/data/TV_PV/Python-converted_regress_141024.xlsx', index=False)
...
save_file_directory = os.path.join(current_file_directory, '../data/eol_intermediate_files/tv.xlsx')
