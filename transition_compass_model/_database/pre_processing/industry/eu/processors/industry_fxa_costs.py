...
def make_costs_dm(current_file_directory, lever_file, years_ots):
    # get cost data
    ...
    df_price_extra = pd.DataFrame(
        {"Country": ["EU28"], "Years": [2015], "ots_tec_price-indices_pli[%]": [100]}
    )
    ...
    for c in countries:
        ...
        df_temp["Country"] = c
        df_costs_new = pd.concat([df_costs_new, df_temp]).reset_index(
            level=0, drop=True
        )
    ...
    for v in variabs:
        df_costs_new[v] = pd.to_numeric(df_costs_new[v], errors="coerce")
        df_costs_new[v] = (
    ...
    for i in range(0, len(tech_current)):
        df1.loc[df1["technology_code"] == tech_current[i], "technology_code"] = (
            tech_new[i]
        )
    ...
    # change EU28 to EU27
    dm.rename_col(["EU28"], ["EU27"], "Country")
    dm_cc.rename_col(["EU28"], ["EU27"], "Country")
...
def run(years_ots):
    ...
    lever_file = "fxa_costs.pickle"
    filepath = os.path.join(current_file_directory, "../data/datamatrix/" + lever_file)
    if os.path.exists(filepath):
        with open(filepath, "rb") as handle:
            DM_costs = pickle.load(handle)
    else:
        DM_costs = make_costs_dm(current_file_directory, lever_file, years_ots)

    dm_costs = DM_costs["costs"].copy()
    dm_costs_cc = DM_costs["costs-cc"].copy()

    return dm_costs, dm_costs_cc
