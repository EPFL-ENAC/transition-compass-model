...
def make_material_switch_dm(current_file_directory, lever_file, years_ots):
    # create dm
    countries = [
        "Austria",
        "Belgium",
...
def run(years_ots):
    ...
    lever_file = "lever_material-switch.pickle"
    filepath = os.path.join(current_file_directory, "../data/datamatrix/" + lever_file)
    if os.path.exists(filepath):
        with open(filepath, "rb") as handle:
            dm = pickle.load(handle)
    else:
        dm = make_material_switch_dm(current_file_directory, lever_file, years_ots)
