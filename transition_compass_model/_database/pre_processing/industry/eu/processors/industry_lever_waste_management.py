...
from transition_compass_model._database.pre_processing.industry.eu.get_data_functions.data_waste import (
    get_waste_buildings_data,
    get_waste_vehicle_data,
)
...
def make_transport(years_ots):
    ###############################################
    ##### GET DATA ON END OF LIFE OF VEHICLES #####
    ###############################################

    dm_elv = get_waste_vehicle_data()
    ...
    def adjust_shares(shares):
        ...
        if discrepancy != 0:
            ...
            idx_to_adjust = max(
                range(len(rounded_shares)), key=lambda i: rounded_shares[i]
            )
    ...
    years_after_regulation = list(
        range(2019, 2023 + 1)
    )  # + list(range(2025, 2050 + 5, 5))
...
def make_buildings(DM_wst_mgt, years_ots):
    dm_bld = get_waste_buildings_data()
    ...
    dm_bld_tot.add(
        0, col_label="waste-uncollected", dummy=True, dim="Variables", unit="t"
    )
...
def make_rails(DM_wst_mgt, years_ots):
...
def make_trolley_cables(DM_wst_mgt, years_ots):
    ###############################################################################
    ################################ TROLLEY CABLES ###############################
    ###############################################################################
...
def make_appliances(DM_wst_mgt, years_ots):
    ####################
    ##### GET DATA #####
    ####################
    ...
    # select countries
    df = df.loc[
        :, ["geoscale", "year", "variable", "wst_oper", "waste", "unit", "value"]
    ]
    country_list = {
        "AT": "Austria",
        "BE": "Belgium",
    ...
    df.loc[df["reuse"].isnull(), "reuse"] = (
        df.loc[df["reuse"].isnull(), "recovery"]
    ...
    df_mun["incineration-coeff"] = df_mun["DSP_I"] / (
        df_mun["DSP_I"] + df_mun["DSP_L_OTH"]
    )
    df_mun["landfill-coeff"] = df_mun["DSP_L_OTH"] / (
        df_mun["DSP_I"] + df_mun["DSP_L_OTH"]
    )
    ...
    # check
    check = (
        df["waste-collected"] != df["recovery"] + df["incineration"] + df["landfill"]
    )
    ...
    dm_domapp.add(
        np.nan, col_label=list(range(2019, 2023 + 1, 1)), dummy=True, dim="Years"
    )
    ...
    idx = dm_domapp_tot.idx
    arr_temp = (
        dm_domapp_tot.array[:, :, :, idx["waste-collected"], np.newaxis] / 0.8 * 0.2
    )
    dm_domapp_tot.add(
        arr_temp, dim="Categories1", col_label="waste-uncollected", unit="t"
    )
    ...
    DM_wst_mgt["electronics-total"] = dm_domapp_tot.filter(
...
def make_packages(DM_wst_mgt, years_ots):
    #####################################################
    ################### PACKAGE WASTE ###################
    #####################################################
    ...
    df["landfill"] = np.round(
        (df["generated"] - df["recovery"]) * df["landfill-coeff"], 0
    )
    ...
    df["variable"] = [
        w + "_" + v + "[t]" for w, v in zip(df["variable"], df["wst-oper"])
    ]
    ...
    perc_uncollected = (
        waste_generated_2023 - waste_treated_2023
    ...
    dm_pack_tot.add(
        arr_temp, col_label="waste-uncollected", dim="Categories1", unit="t"
    )
...
def clean_up(DM_wst_mgt, years_ots):
    ########################################################
    ################### RENAME VARIABLES ###################
    ########################################################
...
def make_waste_management_dm(current_file_directory, lever_file, years_ots):
    # vehicles
    DM_wst_mgt = make_transport(years_ots)

    # buildings
    make_buildings(DM_wst_mgt, years_ots)

    # roads
    make_roads(DM_wst_mgt)

    # rails
    make_rails(DM_wst_mgt, years_ots)

    # trolley cables
    make_trolley_cables(DM_wst_mgt, years_ots)

    # larger appliances, pcs and electronics
    make_appliances(DM_wst_mgt, years_ots)

    # packages
    make_packages(DM_wst_mgt, years_ots)

    # clean up
    dm_ots = clean_up(DM_wst_mgt, years_ots)

    # save
    f = os.path.join(current_file_directory, "../data/datamatrix/", lever_file)
    with open(f, "wb") as handle:
        pickle.dump(dm_ots, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def run(years_ots):
    ...
    lever_file = "lever_waste-management.pickle"
    filepath = os.path.join(current_file_directory, "../data/datamatrix/" + lever_file)
    if os.path.exists(filepath):
        with open(filepath, "rb") as handle:
            dm = pickle.load(handle)
    else:
        dm = make_waste_management_dm(current_file_directory, lever_file, years_ots)
