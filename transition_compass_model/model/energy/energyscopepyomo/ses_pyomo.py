...
def build_model(data, objective="gwp"):
    ...
    m.END_USES_TYPES_OF_CATEGORY = pyo.Set(
        m.END_USES_CATEGORIES, initialize=lambda m, cat: eutoc_map.get(cat, [])
    )
    ...
    m.TECHNOLOGIES_OF_END_USES_TYPE = pyo.Set(
        m.END_USES_TYPES, initialize=lambda m, ty: teu_map.get(ty, [])
    )
    ...
    m.NON_STORAGE_X = pyo.Set(
        initialize=sorted(
            (set(m.RESOURCES) | set(m.TECHNOLOGIES)) - set(m.STORAGE_TECH)
        )
    )
    m.X_TECH_RES = pyo.Set(
        initialize=sorted(
            (set(m.RESOURCES) | set(m.TECHNOLOGIES)) - set(m.STORAGE_TECH)
        )
    )
    ...
    if "end_uses_input" in P:
        ...
        m.end_uses_input = pyo.Param(
            m.END_USES_INPUT, initialize=_eui_init, mutable=True
        )

    m.i_rate = pyo.Param(mutable=True, initialize=P.get("i_rate", 0.04))
    m.share_mobility_public_min = pyo.Param(
        mutable=True, initialize=P.get("share_mobility_public_min", 0.0)
    )
    m.share_mobility_public_max = pyo.Param(
        mutable=True, initialize=P.get("share_mobility_public_max", 1.0)
    )
    m.share_freight_train_min = pyo.Param(
        mutable=True, initialize=P.get("share_freight_train_min", 0.0)
    )
    m.share_freight_train_max = pyo.Param(
        mutable=True, initialize=P.get("share_freight_train_max", 1.0)
    )
    m.share_heat_dhn_min = pyo.Param(
        mutable=True, initialize=P.get("share_heat_dhn_min", 0.0)
    )
    m.share_heat_dhn_max = pyo.Param(
        mutable=True, initialize=P.get("share_heat_dhn_max", 1.0)
    )
    ...
    m.ref_size = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("ref_size", {})
    )
    m.c_inv = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("c_inv", {})
    )
    m.c_maint = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("c_maint", {})
    )
    m.lifetime = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=1.0, initialize=P.get("lifetime", {})
    )
    m.f_max = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("f_max", {})
    )
    m.f_min = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("f_min", {})
    )
    m.fmax_perc = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=1.0, initialize=P.get("fmax_perc", {})
    )
    m.fmin_perc = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("fmin_perc", {})
    )
    ...
    m.c_p = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=1.0, initialize=P.get("c_p", {})
    )
    m.gwp_constr_param = pyo.Param(
        m.TECHNOLOGIES, mutable=True, default=0.0, initialize=P.get("gwp_constr", {})
    )
    ...
    m.avail = pyo.Param(
        m.RESOURCES, mutable=True, default=0.0, initialize=P.get("avail", {})
    )
    m.gwp_op_param = pyo.Param(
        m.RESOURCES, mutable=True, default=0.0, initialize=P.get("gwp_op", {})
    )
    ...
    # Losses & peaks
    m.loss_coeff = pyo.Param(
        m.END_USES_TYPES, mutable=True, default=0.0, initialize=P.get("loss_coeff", {})
    )
    ...
    m.Storage_In = pyo.Var(
        m.STORAGE_TECH, m.LAYERS, m.PERIODS, domain=pyo.NonNegativeReals
    )
    m.Storage_Out = pyo.Var(
        m.STORAGE_TECH, m.LAYERS, m.PERIODS, domain=pyo.NonNegativeReals
    )

    m.Share_Mobility_Public = pyo.Var(
        bounds=(
            pyo.value(m.share_mobility_public_min),
            pyo.value(m.share_mobility_public_max),
        )
    )
    m.Share_Freight_Train = pyo.Var(
        bounds=(
            pyo.value(m.share_freight_train_min),
            pyo.value(m.share_freight_train_max),
        )
    )
    m.Share_Heat_Dhn = pyo.Var(
        bounds=(pyo.value(m.share_heat_dhn_min), pyo.value(m.share_heat_dhn_max))
    )
    ...
    def end_uses_rule(m, l, t):
        if l == "ELECTRICITY":
            return (
                m.End_Uses[l, t]
                == (
                    (m.end_uses_input[l] / m.total_time)
                    + (m.end_uses_input["LIGHTING"] * m.lighting_month[t] / m.t_op[t])
                )
                + m.Losses[l, t]
            )
            ...
            return (
                m.End_Uses[l, t]
                == (
                    (m.end_uses_input["HEAT_LOW_T_HW"] / m.total_time)
                    + (
                        m.end_uses_input["HEAT_LOW_T_SH"]
                        * m.heating_month[t]
                        / m.t_op[t]
                    )
                )
            ...
            return (
                m.End_Uses[l, t]
                == (m.end_uses_input["MOBILITY_PASSENGER"] / m.total_time)
                * m.Share_Mobility_Public
            )
        elif l == "MOB_PRIVATE":
            return m.End_Uses[l, t] == (
                m.end_uses_input["MOBILITY_PASSENGER"] / m.total_time
            ...
            return (
                m.End_Uses[l, t]
                == (m.end_uses_input["MOBILITY_FREIGHT"] / m.total_time)
                * m.Share_Freight_Train
            )
    ...
    m.number_of_units = pyo.Constraint(
        m.TECHNOLOGIES - m.INFRASTRUCTURE,
        rule=lambda m, i: m.Number_Of_Units[i] == m.F_Mult[i] / m.ref_size[i],
    )

    # [Eq. 1.6] Size limits
    m.size_limit = pyo.Constraint(
        m.TECHNOLOGIES,
        rule=lambda m, i: pyo.inequality(m.f_min[i], m.F_Mult[i], m.f_max[i]),
    )
    ...
    m.capacity_factor_t = pyo.Constraint(
        m.TECHNOLOGIES,
        m.PERIODS,
        rule=lambda m, i, t: m.F_Mult_t[i, t] <= m.F_Mult[i] * m.c_p_t[i, t],
    )
        ...
        rule=lambda m, i, t: (
            ...
            if i == "DEC_SOLAR"
                ...
                * (
                    ...
                    / (
                        m.end_uses_input["HEAT_LOW_T_HW"]
                        + m.end_uses_input["HEAT_LOW_T_SH"]
                    )
                )
            )
    ...
    m.op_strategy_decen_1_linear_2 = pyo.Constraint(
        ...
        rule=lambda m, i, t: (
            pyo.Constraint.Skip
            if i == "DEC_SOLAR"
            else (m.X_Solar_Backup_Aux[i, t] <= m.F_Mult_t["DEC_SOLAR", t])
        ),
    )
    m.op_strategy_decen_1_linear_3 = pyo.Constraint(
        ...
        rule=lambda m, i, t: (
            ...
            if i == "DEC_SOLAR"
                ...
                m.X_Solar_Backup_Aux[i, t]
                >= m.F_Mult_t["DEC_SOLAR", t]
                - (1 - m.Y_Solar_Backup[i]) * m.f_max["DEC_SOLAR"]
            )
        ),
    )

    # [Eq. 1.20] ]Only one backup
    m.op_strategy_decen_2 = pyo.Constraint(
        expr=sum(m.Y_Solar_Backup[i] for i in m.TECHNOLOGIES) <= 1
    )
    ...
    def storage_level_rule(m, i, t):
        ...
        return (
            m.F_Mult_t[i, t] == m.F_Mult_t[i, prev_t] + (inflow - outflow) * m.t_op[t]
        )
    ...
    m.f_max_perc = pyo.Constraint(
        ...
        rule=lambda m, i, j: (
            ...
            if (j not in set(m.TECHNOLOGIES_OF_END_USES_TYPE[i]))
                ...
                sum(m.F_Mult_t[j, t] * m.t_op[t] for t in m.PERIODS)
                <= m.fmax_perc[j]
                * sum(
                    m.F_Mult_t[j2, t2] * m.t_op[t2]
                    for j2 in m.TECHNOLOGIES_OF_END_USES_TYPE[i]
                    for t2 in m.PERIODS
                )
            )
        ),
    ...
    m.extra_power2gas_1 = pyo.Constraint(
        rule=lambda m: m.F_Mult["POWER2GAS_3"] >= m.F_Mult["POWER2GAS_1"]
    )
    m.extra_power2gas_2 = pyo.Constraint(
        rule=lambda m: m.F_Mult["POWER2GAS_3"] >= m.F_Mult["POWER2GAS_2"]
    )
    ...
    m.op_strategy_mob_private = pyo.Constraint(
        ...
        rule=lambda m, i, t: (
            ...
            if i
            not in (
                set(m.TECHNOLOGIES_OF_END_USES_CATEGORY["MOBILITY_PASSENGER"])
                | set(m.TECHNOLOGIES_OF_END_USES_CATEGORY["MOBILITY_FREIGHT"])
            )
            else m.F_Mult_t[i, t]
            >= sum(m.F_Mult_t[i, t2] * m.t_op[t2] / m.total_time for t2 in m.PERIODS)
        ),
    )
    ...
    m.totalcost_cal = pyo.Constraint(
        expr=m.TotalCost
        == sum(m.tau[i] * m.C_inv[i] + m.C_maint[i] for i in m.TECHNOLOGIES)
        + sum(m.C_op[j] for j in m.RESOURCES)
    )

    # ----- Emissions -----
    m.gwp_constr_calc = pyo.Constraint(
        m.TECHNOLOGIES,
        rule=lambda m, i: m.GWP_constr[i] == m.gwp_constr_param[i] * m.F_Mult[i],
    )
    ...
    m.totalGWP_calc = pyo.Constraint(
        expr=m.TotalGWP
        == sum(m.GWP_constr[i] / m.lifetime[i] for i in m.TECHNOLOGIES)
        + sum(m.GWP_op[j] for j in m.RESOURCES)
    )
